...
## Elevator Pitch.
This cloud app pickup a photo from deviceÅfs image galley, save the photo on self hosted WordPress site as a media fileÅCand create a post with the media file attachment.
...
1. Pickup a photo from deviceÅfs image galley.
1. Press ÅgPost to mediaÅh button to save the photo on WordPress site as a media file
1. Type the post main content
1. Press ÅgPost BlogÅh to create a post with the media file attachment.

the app is easy way to make a photo blog to own WordPress site.

The appÅfs social benefit comes from its ability to make a quick post, and improve later, give a motivation to help user more publish. Also this a good example to use [WordPress REST API](http://wp-api.org/).
...
As shown in this screenshot, the app exceeds the projectÅfs minimum technical requirements. It contains 2 global variables, 4 list data, 2 procedures or functions, and 2 if/else structure.

![](./blocksview.png)

## Appendix.

The WP REST API is a WordPress plugin that intends to be eventually integrated into WordPressÅf core as another (big) step towards transforming WordPress from a blogging platform/CMS into a fully fledged application framework.
